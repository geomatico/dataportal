...
public class GnSpeaker {
    ...
    private static final String gnPassword = "la pass del GN";
    ...
    public int ask2gn() throws GNLibException, GNServerException{
		...
		if (parametros.containsKey("variables")){
			logger.info("P VARIABLES: " + arrayToString(parametros.get("variables"), ","));
			// modificar esto para busqueda por campo propio 
			searchRequest.addParam(GNSearchRequest.Param.any, arrayToString(parametros.get("variables"), ","));
		}
		if (parametros.containsKey("daterange")){
			// esta busqueda no da resultado
			searchRequest.addParam(GNSearchRequest.Param.dateFrom, parametros.get("daterange")[0]);
			searchRequest.addParam(GNSearchRequest.Param.dateTo, parametros.get("daterange")[1]);
		}
