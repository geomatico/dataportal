...
<xsl:stylesheet version="2.0"
		xmlns:tds="http://www.unidata.ucar.edu/namespaces/thredds/InvCatalog/v1.0"
		...
		<records>
			<xsl:for-each select="nc:netcdf[1]">
				...
				<record uuid="{$uuid}">
					...
					<fragment id="thredds.metadata.creation" uuid="{util:toString(util:randomUUID())}" title="{concat($name,'_metadata_creation')}">
						<gmd:dateStamp>
							<gco:DateTime>
					...
					<fragment id="thredds.title" uuid="{util:toString(util:randomUUID())}" title="{concat($name,'_title')}">
						<gmd:title>
							<gco:CharacterString>
								<xsl:choose>
									<xsl:when test="nc:attribute[@name='title']/@value != ''"><xsl:value-of select="nc:attribute[@name='title']/@value"/></xsl:when>
					...
					<replacementGroup id="thredds.resource.dates">
						...
						<xsl:if test="$creationDate!=''">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($name,'_resource_creation')}">
								<gmd:date>
									<gmd:CI_Date>
										<gmd:date>
											<gco:DateTime>
						...
						<xsl:if test="$modifiedDate!=''">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($name,'_resource_revision')}">
								<gmd:date>
									<gmd:CI_Date>
										<gmd:date>
											<gco:DateTime>
						...
						<xsl:if test="$issuedDate!=''">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($name,'_resource_published')}">
								<gmd:date>
									<gmd:CI_Date>
										<gmd:date>
											<gco:DateTime>
					...
					<replacementGroup id="thredds.responsible.parties">
						...
						<xsl:variable name="publisher_name" select="nc:attribute[@name='publisher_name']/@value"/>
						<xsl:variable name="publisher_url" select="nc:attribute[@name='publisher_url']/@value"/>
						<xsl:variable name="publisher_email" select="nc:attribute[@name='publisher_email']/@value"/>
						
						<xsl:if test="$publisher_name!=''">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($name,'_publisher')}">
														...
														<gmd:address>
															<gmd:CI_Address>
																<gmd:electronicMailAddress>
																	<gco:CharacterString><xsl:value-of select="$publisher_email"/></gco:CharacterString>
																</gmd:electronicMailAddress>
															</gmd:CI_Address>
														</gmd:address>
													</xsl:if>
													<xsl:if test="$publisher_url!=''">
														<gmd:onlineResource>
															<gmd:CI_OnlineResource>
																<gmd:linkage>
																	<gmd:URL><xsl:value-of select="$publisher_url"/></gmd:URL>
																</gmd:linkage>
																<gmd:protocol>
						...
						<xsl:if test="$creator_name!='' or $institution!=''">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($name,'_originator')}">
								<gmd:citedResponsibleParty>
									<gmd:CI_ResponsibleParty>
										<xsl:if test="$creator_name!=''">
											<gmd:individualName>
												<gco:CharacterString><xsl:value-of select="$creator_name"/></gco:CharacterString>
											</gmd:individualName>
										</xsl:if>
										<xsl:if test="$institution!=''">
											<gmd:organisationName>
												<gco:CharacterString><xsl:value-of select="$institution"/></gco:CharacterString>
											</gmd:organisationName>
										</xsl:if>
												...
												<gmd:CI_Contact>
													<xsl:if test="$creator_email!=''">
														<gmd:address>
															<gmd:CI_Address>
																<gmd:electronicMailAddress>
																	<gco:CharacterString><xsl:value-of select="$creator_email"/></gco:CharacterString>
																</gmd:electronicMailAddress>
															</gmd:CI_Address>
														</gmd:address>
													</xsl:if>
													<xsl:if test="$creator_url!=''">
														<gmd:onlineResource>
															<gmd:CI_OnlineResource>
																<gmd:linkage>
																	<gmd:URL><xsl:value-of select="$creator_url"/></gmd:URL>
																</gmd:linkage>
																<gmd:protocol>
																	<gco:CharacterString>WWW:LINK-1.0-http--link</gco:CharacterString>
																</gmd:protocol>
						...
						<xsl:variable name="contributor_name" select="nc:attribute[@name='contributor_name']/@value"/>
						<xsl:variable name="contributor_role" select="nc:attribute[@name='contributor_role']/@value"/>
					...
					<fragment id="thredds.abstract" uuid="{util:toString(util:randomUUID())}" title="{concat($name,'_abstract')}">
						<gmd:abstract>
							<gco:CharacterString>
								<xsl:choose>
					...
					<replacementGroup id="thredds.credit">
						<xsl:if test="$acknowledgment!=''">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($name,'_credit')}">
								<gmd:credit>
									<gco:CharacterString><xsl:value-of select="$acknowledgment"/></gco:CharacterString>
								</gmd:credit>
							</fragment>
						</xsl:if>
					</replacementGroup>
					...
					<xsl:variable name="keywords_vocabulary" select="nc:attribute[@name='keywords_vocabulary']/@value"/>
					...
					<replacementGroup id="thredds.use.constraints">
						<xsl:if test="$license!=''">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($name,'_use_constraints')}">
					...
					<replacementGroup id="thredds.project">
						<xsl:if test="$project!=''">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($name,'_project')}">
								<gmd:aggregationInfo>
									<gmd:MD_AggregateInformation>
										<gmd:aggregateDataSetName>
											<gmd:CI_Citation>
												<gmd:title>
													<gco:CharacterString><xsl:value-of select="$project"/></gco:CharacterString>
												</gmd:title>
												<gmd:date>
													<gmd:CI_Date>
														<gmd:dateType>
										...
										<gmd:initiativeType>
											<gmd:DS_InitiativeTypeCode codeList="http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#DS_InitiativeTypeCode" codeListValue="project">project</gmd:DS_InitiativeTypeCode>
										</gmd:initiativeType>
									</gmd:MD_AggregateInformation>
								</gmd:aggregationInfo>
							</fragment>
					...
					<fragment id="thredds.extent" uuid="{util:toString(util:randomUUID())}" title="{concat($name,'_extent')}">
						<gmd:extent>
							<gmd:EX_Extent>
			
								<!-- geographic extent [0..1] -->
								
								<xsl:variable name="geospatial_lat_min" select="nc:attribute[@name='geospatial_lat_min']/@value"/>
								<xsl:variable name="geospatial_lat_max" select="nc:attribute[@name='geospatial_lat_max']/@value"/>
								<xsl:variable name="geospatial_lon_min" select="nc:attribute[@name='geospatial_lon_min']/@value"/>
								<xsl:variable name="geospatial_lon_max" select="nc:attribute[@name='geospatial_lon_max']/@value"/>
								
								<xsl:if test="$geospatial_lat_min!='' or $geospatial_lat_max!='' or $geospatial_lon_min!='' or $geospatial_lon_max!=''">
								...
								<xsl:variable name="time_coverage_start" select="nc:attribute[@name='time_coverage_start']/@value"/>
								<xsl:variable name="time_coverage_end" select="nc:attribute[@name='time_coverage_end']/@value"/>
								<xsl:variable name="time_coverage_duration" select="nc:attribute[@name='time_coverage_duration']/@value"/>
								<xsl:variable name="time_coverage_resolution" select="nc:attribute[@name='time_coverage_resolution']/@value"/>
								...
								<xsl:variable name="geospatial_vertical_min" select="nc:attribute[@name='geospatial_vertical_min']/@value"/>
								<xsl:variable name="geospatial_vertical_max" select="nc:attribute[@name='geospatial_vertical_max']/@value"/>
								
								<xsl:if test="$geospatial_vertical_min!='' or $geospatial_vertical_max!=''">
									<gmd:verticalElement>
										<gmd:EX_VerticalExtent>
											<gmd:minimumValue>
												<gco:Real><xsl:value-of select="$geospatial_vertical_min"/></gco:Real>
											</gmd:minimumValue>
											<gmd:maximumValue>
												<gco:Real><xsl:value-of select="$geospatial_vertical_max"/></gco:Real>
											</gmd:maximumValue>
										</gmd:EX_VerticalExtent>
									</gmd:verticalElement>
								</xsl:if>
					...
					<replacementGroup id="thredds.supplemental">
						<xsl:if test="$comment!=''">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($name,'_supplemental')}">
					...
					<replacementGroup id="thredds.contentinfo">
					...
					<xsl:for-each select="//nc:variable">
					<gmd:dimension>
					 <gmd:MD_Band>
					...
					<gco:aName>
					<xsl:choose>
						<xsl:when test="not(nc:attribute[@name='standard_name'])">
							<gco:CharacterString><xsl:value-of select='@name' /></gco:CharacterString>
						</xsl:when>
						<xsl:otherwise>
							<gco:CharacterString><xsl:value-of select="nc:attribute[@name='standard_name']/@value" /></gco:CharacterString>
						</xsl:otherwise>
					</xsl:choose> 					  
					</gco:aName> 
					<gco:attributeType> 
					...
					<gco:CharacterString><xsl:value-of select='@type' /></gco:CharacterString> 
					  ...
					  <gmd:descriptor>
					    <gco:CharacterString><xsl:value-of select="nc:attribute[@name='long_name']/@value" /></gco:CharacterString>
					  </gmd:descriptor>
					 </gmd:MD_Band> 
					</gmd:dimension> 
					</xsl:for-each> 
					...
					<replacementGroup id="thredds.online.resources">
						...
						<xsl:for-each select="//tds:serviceName|//@serviceName">
							...
							<xsl:apply-templates select="/root/tds:catalog//tds:service[@name=$serviceName]" mode="transfer-options"/>
						</xsl:for-each>
											
						<!-- Link to thredds catalog for dataset -->
						
						<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($name,'_dataset_link')}">
							<gmd:onLine>
								<gmd:CI_OnlineResource>
									<gmd:linkage>
										<gmd:URL>
											<xsl:value-of select="concat($catalogUrl,'?dataset=',/root/tds:catalog/tds:dataset/@ID)"/>
										</gmd:URL>
									</gmd:linkage>
									<gmd:protocol>
					...
					<replacementGroup id="thredds.data.quality">
						<xsl:if test="$history!=''">
							<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($name,'_history')}">
								<gmd:dataQualityInfo>
									<gmd:DQ_DataQuality>
										<gmd:scope>
											<gmd:DQ_Scope>
										...
										<gmd:lineage>
											<gmd:LI_Lineage>
												<gmd:statement>
													<gco:CharacterString><xsl:value-of select="$history"/>	</gco:CharacterString>
												</gmd:statement>
											</gmd:LI_Lineage>
										</gmd:lineage>
									</gmd:DQ_DataQuality>
						...
						<xsl:variable name="processing_level" select="nc:attribute[@name='processing_level']/@value"/>
							...
							<gmd:dataQualityInfo>
								<gmd:DQ_DataQuality>
									...
									<gmd:lineage>
										<gmd:LI_Lineage>
											<gmd:statement>
												<gco:CharacterString><xsl:value-of select="$processing_level"/></gco:CharacterString>
											</gmd:statement>
										</gmd:LI_Lineage>
									</gmd:lineage>
								</gmd:DQ_DataQuality>
				...
				<fragment uuid="{util:toString(util:randomUUID())}" title="{concat($name,'_service_url')}">
					<gmd:onLine>
						<gmd:CI_OnlineResource>
							...
							<gmd:description>
								<gco:CharacterString>Data available via <xsl:value-of select="@serviceType"/></gco:CharacterString>
							</gmd:description>
						</gmd:CI_OnlineResource>
					</gmd:onLine>
				</fragment>
